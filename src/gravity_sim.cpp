...
public:
    ...
    bool Launched = false;
    vector<float> hue = {1.0f, 0.0f, 0.0f, 1.0f};
    float mass;
    float density = 0.08375; // kg / m^3  HYDROGEN
    float radius = pow(((3 * this->mass / this->density) / (4 * 3.14159265359)), (1.0f / 3.0f));

    Object(vector<float> initPosition, vector<float> initVelocity, float mass)
    {
    ...
    {
        vector<float> xy = {this->position[0], this->position[1]};
        return xy;
    }
    void Draw()
    {
    ...
    void GetData()
    {
        cout << "Xpos: " << this->position[0] << " Ypos: " << this->position[1] << endl
             << "Xvel: " << this->velocity[0] << " Yvel: " << this->velocity[1] << endl
             << endl;
    }
    ...
    void CheckCollision(vector<Object> &objs)
    {
        for (auto &obj2 : objs)
        {
            if (&obj2 == this)
...
vector<float> initVelocity = {0.0f, 0.0f};
vector<Object> objs = {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (auto &obj : objs)
        {
            ...
            for (auto &obj2 : objs)
            {
                ...
                if (distance != 0)
                {
                    vector<float> direction = {(obj2.GetCoord()[0] - obj.GetCoord()[0]) / distance, (obj2.GetCoord()[1] - obj.GetCoord()[1]) / distance};
            ...
            if (obj.Launched)
            {
                ...
                if (distance != 0)
                {
                    vector<float> direction = {(float(xpos) - obj.GetCoord()[0]) * distance * 0.01f, (float(ypos) - obj.GetCoord()[1]) * distance * 0.01f};
                    cout << direction[0] << ' ' << direction[1] << endl;
...
void mouseButtonCallback(GLFWwindow *window, int button, int action, int mods)
{
    if (button == GLFW_MOUSE_BUTTON_LEFT)   
    {
        if (action == GLFW_PRESS)
        {
            ...
            glfwGetCursorPos(window, &xpos, &ypos);
            ypos = screenHeight - ypos;
            objs.emplace_back(vector<float>{float(xpos), float(ypos)}, vector<float>{0.0f, 0.0f}, initMass);
...
void LiveTitle(GLFWwindow *window, double &mouseX, double &mouseY)
{
    ...
    string windowTitle = string(title) + " | Mouse X: " + to_string(mouseX) + " Y: " + to_string(mouseY);
